...
let str_of_type ~options ~path ({ ptype_loc = loc } as type_decl) =
  ...
  let init_cases =
        ...
        Exp.case [%pat? "-h" :: rest] [%expr [%e evar perrfun_name] default; Ppx_deriving_runtime.exit 2];
        Exp.case [%pat? "-help" :: rest] [%expr [%e evar perrfun_name] default; Ppx_deriving_runtime.exit 2];
        Exp.case [%pat? "--" :: rest] [%expr (cfg, rest)]] in
    ...
    let creator, cases, options =
        List.fold_right (fun ({ pld_name = { txt = name; loc }; pld_type; pld_attributes } as pld) (msgs, cases, options) ->
          ...
          let case = match pld_type with
              (* | [%type: bool] -> Exp.record [lid name, attr_set_false pld_attributes] (Some [%expr cfg]) *)
              | [%type: bool] ->
          ...
          msg :: msgs, case :: cases, str option :: options) labels ([help_msg], error_cases, []) in
